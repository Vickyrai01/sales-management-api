...
public class SaleController {
    ...
    @GetMapping
    @Operation(summary = "Get all sales", description = "Retrieve a list of all sales. Required role: SELLER")
    public ResponseEntity<List<SaleDTO>> getAllSales(){
        return ResponseEntity.ok(saleService.getAllSales());
    }

    @GetMapping("/{id}")
    @Operation(summary = "Get sale by ID", description = "Retrieve a specific sale by its ID. Required role: SELLER")
    @ApiResponses(value = {
    ...
    @PostMapping
    @Operation(summary = "Create a new sale", description = "Create a new sale. Required role: SELLER")
    @ApiResponses(value = {
    ...
    @PutMapping("/{id}")
    @Operation(summary = "Update a sale", description = "Update an existing sale by ID. Required role: SELLER")
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Sale updated successfully"),
        @ApiResponse(responseCode = "404", description = "Sale, Branch, or Product not found", 
            content = @Content(schema = @Schema(implementation = ErrorMessage.class)))
    })
    ...
    @DeleteMapping("/{id}")
    @Operation(summary = "Delete a sale", description = "Delete a sale by ID. Required role: SELLER")
    @ApiResponses(value = {
    ...
    @Operation(summary = "Confirm a sale", description = "Confirm a sale and update stock. Required role: SELLER")
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Sale confirmed successfully"),
        @ApiResponse(responseCode = "404", description = "Sale not found", 
            content = @Content(schema = @Schema(implementation = ErrorMessage.class))),
        @ApiResponse(responseCode = "409", description = "Invalid state, insufficient stock, or product not available in branch", 
            content = @Content(schema = @Schema(implementation = ErrorMessage.class)))
    })
    ...
    @PostMapping("/{id}/pay")
    @Operation(summary = "Mark a sale as paid", description = "Mark a confirmed sale as paid. Required role: SELLER")
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Sale marked as paid successfully"),
        @ApiResponse(responseCode = "404", description = "Sale not found", 
            content = @Content(schema = @Schema(implementation = ErrorMessage.class))),
        @ApiResponse(responseCode = "409", description = "Invalid state - sale must be confirmed before payment", 
            content = @Content(schema = @Schema(implementation = ErrorMessage.class)))
    })
    ...
    @Operation(summary = "Cancel a sale", description = "Cancel a pending or confirmed sale. Required role: SELLER")
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Sale cancelled successfully"),
        @ApiResponse(responseCode = "404", description = "Sale not found", 
            content = @Content(schema = @Schema(implementation = ErrorMessage.class))),
        @ApiResponse(responseCode = "409", description = "Invalid state - cannot cancel a paid or already cancelled sale", 
            content = @Content(schema = @Schema(implementation = ErrorMessage.class)))
    })
